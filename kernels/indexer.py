...
class DeepseekV32IndexerMetadataBuilder(AttentionMetadataBuilder):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # +1 for MTP past max_model_len; +next_n margin for concurrent multi-seq
        # decode (block tables from the runner can disagree by 1 with pure cdiv).
        _spec = int(self.num_speculative_tokens or 0)
    ...
    def _prepare_decode_tensors(
        ...
        if not use_native and max_decode_len > 1:
            ...
            if min_decode_len == max_decode_len:
                ...
                # original request. Width may differ by 1 vs preallocated buffer
                # (runner page-table vs cdiv(max_model_len)+margin) — copy min cols.
                expanded_bt = torch.repeat_interleave(
                    block_table, decode_lens, dim=0, output_size=actual_expanded
                )
                buf = self.expanded_block_table_buffer
                copy_cols = min(expanded_bt.shape[1], buf.shape[1])
                buf[:actual_expanded, :copy_cols] = expanded_bt[:, :copy_cols]
                if copy_cols < buf.shape[1]:
                    buf[:actual_expanded, copy_cols:] = 0
                if actual_expanded < num_decode_tokens:
                    buf[actual_expanded:num_decode_tokens].zero_()
                block_table = buf[:num_decode_tokens]
