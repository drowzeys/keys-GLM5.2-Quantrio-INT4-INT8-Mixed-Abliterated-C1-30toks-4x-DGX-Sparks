...
class DeepSeekMultiTokenPredictorLayer(nn.Module):
    def __init__(self, vllm_config: VllmConfig, prefix: str) -> None:
        ...
        # GB10 fix: get_draft_quant_config() bypasses configure_quant_config(), so the
        # draft quant_config carries no packed_modules_mapping and compressed-tensors
        # cannot resolve the fused q/kv-A projection -> the mtp_block's fused_qkv_a_proj
        # is built UNQUANTIZED and loading QuantTrio Int4 (...fused_qkv_a_proj.weight_packed)
        # KeyErrors. Attach the mapping (mirrors DeepseekV2ForCausalLM) so the block's
        # fused projection is built packed-quantized and loads correctly.
        if quant_config is not None and getattr(config, "q_lora_rank", None) is not None:
            pmm = dict(getattr(quant_config, "packed_modules_mapping", {}) or {})
            pmm.setdefault("gate_up_proj", ["gate_proj", "up_proj"])
            pmm["fused_qkv_a_proj"] = ["q_a_proj", "kv_a_proj_with_mqa"]
            quant_config.packed_modules_mapping = pmm
        ...
        if self.is_v32:
            ...
            # GB10 fix: allocate the DCP scores buffer whenever the b12x indexer is
            # active. The draft's parallel_config.decode_context_parallel_size is 1
            # (draft_tensor_parallel_size=1), but under VLLM_DCP_SHARD_DRAFT the draft
            # decodes on the verifier's DCP group (world>1), so the b12x decode path
            # needs topk_scores_buffer. Gating on the draft's DCP=1 left it None -> crash.
...
class DeepSeekMTP(nn.Module, DeepseekV2MixtureOfExperts):
    # GB10 fix: mirror DeepseekV2ForCausalLM's packed_modules_mapping so the DRAFT
    # (MTP) quantization config can resolve fused modules. configure_quant_config()
    # passes model_class.packed_modules_mapping into the draft quant_config BY
    # REFERENCE; without fused_qkv_a_proj here, the mtp_block's fused q/kv-A
    # projection is built UNQUANTIZED and loading the QuantTrio Int4 checkpoint's
    # `...fused_qkv_a_proj.weight_packed` KeyErrors (kv_a_proj_with_mqa.weight_packed).
